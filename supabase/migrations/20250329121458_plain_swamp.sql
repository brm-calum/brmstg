/*
  # Fix send_email function to use correct http_request structure

  1. Changes
    - Updates send_email function to match http_request type definition
    - Uses proper http_method type for method field
    - Correctly formats headers array
...
BEGIN
  ...
  BEGIN
    ...
    v_headers := ARRAY[
      ('Authorization', 'Basic ' || v_auth)::extensions.http_header,
      ('Content-Type', 'application/json')::extensions.http_header
    ];

    -- Send email using Mailjet API with properly formatted http_request
    SELECT content::json INTO v_response
    FROM http((
      'POST'::http_method,                      -- method
      v_url::character varying,                 -- uri
      v_headers,                                -- headers
      'application/json'::character varying,     -- content_type
      v_payload::character varying              -- content
    )::http_request);
