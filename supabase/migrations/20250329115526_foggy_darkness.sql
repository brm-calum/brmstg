/*
  # Fix HTTP Request Type Schema

  1. Changes
    - Creates http_request type in extensions schema
    - Updates send_email function to use proper type casting
...
-- Create http_request type in extensions schema
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'http_request' AND typnamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'extensions')) THEN
...
-- Update send_email function to use extensions.http_request
