...
import { Building2, MessageSquare, Plus, Warehouse, FileText, LayoutDashboard } from 'lucide-react';
...
export function DashboardPage() {
  ...
  const { unreadCount, refresh: refreshMessages } = useBookingMessages();
  ...
  const setupSubscriptions = useCallback(() => {
    if (!user) {
      logDebug({
        function_name: 'setupSubscriptions',
    ...
    const inquiriesChannel = supabase
      ...
      .on(
        ...
        () => {
          logDebug({
            function_name: 'inquiriesChannel',
            input_params: { event: 'change' }
          });
          loadStats();
        }
      )
    ...
    if (isWarehouseOwner || isAdmin) {
      const warehousesChannel = supabase
        ...
        .on(
          ...
          () => {
            logDebug({
              function_name: 'warehousesChannel',
              input_params: { event: 'change' }
            });
            loadStats();
          }
  ...
  }, [user, isAdmin, isTrader, isWarehouseOwner]);
  ...
  }, [user, loadStats, refreshMessages, setupSubscriptions]);
